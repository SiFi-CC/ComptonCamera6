...
std::tuple<double, double, double> BinnedGeometry::getBinCenter(int x, int y, int z)
{
    auto segXSize = (xRange.second - xRange.first) / binX;
    auto segYSize = (yRange.second - yRange.first) / binY;
    auto segZSize = (zRange.second - zRange.first) / binZ;

    return std::make_tuple(xRange.first + segXSize * (x - 0.5), yRange.first + segYSize * (y - 0.5),
                           zRange.first + segZSize * (z - 0.5));
}

bool BinnedGeometry::isValidBin(std::tuple<int, int, int> bin)
{
    return std::get<0>(bin) > 0 && std::get<0>(bin) <= binX && std::get<1>(bin) > 0 &&
           std::get<1>(bin) <= binY && std::get<2>(bin) > 0 && std::get<2>(bin) <= binZ;
}

bool BinnedGeometry::isInside(std::tuple<double, double, double> pos)
{
    return std::get<0>(pos) >= xRange.first && std::get<0>(pos) <= xRange.second &&
           std::get<1>(pos) >= yRange.first && std::get<1>(pos) <= yRange.second &&
