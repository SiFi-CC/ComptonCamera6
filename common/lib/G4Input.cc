...
G4Input::G4Input(TString InputFile, Bool_t verb)
    : verbose(verb), fEventCo(0), fEventCoReal(0), fEventCoReco(0), fEventNo(0),
      fPrimaryEn(0), fEnergy_e(0), fEnergy_p(0), fIden(false), fScatterer_x(0),
      fScatterer_y(0), fScatterer_z(0), fAbsorber_x(0), fAbsorber_y(0),
      fAbsorber_z(0) {
  ...
  if (InputFile != "") {
    ...
    if (fInputFile->IsZombie() != true) {
      ...
      fInputTree_setup->SetBranchAddress("ScattererPosition",
                                         &fPosition_scatterer);
      fInputTree_setup->SetBranchAddress("AbsorberPosition",
                                         &fPosition_absorber);
      ...
      fInputTree_reco =
          (TTree*)fInputFile->Get("G4SimulationData_Reconstruction");
      ...
      fInputTree_reco->SetBranchAddress("RecoClusterPositions",
                                        &fReco_cluster_pos);
      fInputTree_reco->SetBranchAddress("RecoClusterEnergies",
                                        &fReco_cluster_en);
    } else {
      std::cout << "The inputfile with the name " << InputFile
                << " is zombie! Abort!" << std::endl;
      std::abort();
    }
  } else {
    std::cout
        << "The name of the inputfile for the G4Input is not defined! Abort!"
        << std::endl;
    std::abort();
  }
}
...
Bool_t G4Input::LoadEvent(Int_t no, Bool_t real = false, Bool_t iden = false) {
  ...
  if (error) {
    if (no = 0)
      std::cerr << "No event with the requested specification found!"
                << std::endl;
    return false;
  } else
    return true;
