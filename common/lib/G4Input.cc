...
fAbsorber_z(0) 
{
	...
	if(InputFile!=""){
		...
		if(fInputFile->IsZombie()!=true){	
			...
			std::cout << "The inputfile with the name " << InputFile << " is zombie! Abort!" << std::endl;
			std::abort(); 
		}
	}
	else{
		std::cout << "The name of the inputfile for the G4Input is not defined! Abort!" << std::endl;
		std::abort(); 
	}
}
...
Bool_t G4Input::LoadEvent(Bool_t real=false,Bool_t iden=false){
	if(real==false){
		if(iden==false){
			return LoadEvent(fEventCo,real,iden);
		}
		else{
	...
	if(real==false){
		if(iden==false){
			...
			for(int i=no;i<fInputTree_reco->GetEntries();i++){
				...
				if(fIden!=false){
					if(verbose){
						std::cout << "Identified is requested. The loaded event has the event number " << fEventNo << "." << std::endl;
					}
		...
		for(int i=no;i<fInputTree_real->GetEntries();i++){
			...
			if(fEnergy_e!=0){
				if(iden==false){
					if(verbose){
						std::cout << "Real event is requested. The loaded event has the event number " << fEventNo << "." << std::endl;
					}
					fInputTree_reco->GetEntry(i);
					fEventCoReal=i+1;
					error=false;
					...
					if(fIden!=false){
						if(verbose){
							std::cout << "Real event and identified is requested. The loaded event has the event number " << fEventNo << "." << std::endl;
						}
						fEventCoReal=i+1;
						error=false;
						break;
...
void G4Input::Reset(){
	if(verbose){
		std::cout << "The G4Fileloader is reseted!" << std::endl;
	}
	fEventCo=0;
	fEventCoReal=0;
	fEventCoReco=0;
...
Int_t G4Input::GetNumberOfRealEvents(){
	return fInputTree_real->GetEntries("RealEnergy_e!=0");
}

Int_t G4Input::GetNumberOfEvents(){
	return fInputTree_reco->GetEntries();
}
...
Double_t G4Input::GetScattererXLength(){
	return fScatterer_x;
}

Double_t G4Input::GetScattererYLength(){
	return fScatterer_y;
}

Double_t G4Input::GetScattererZLength(){
	return fScatterer_z;
}

Double_t G4Input::GetAbsorberXLength(){
	return fAbsorber_x;
}

Double_t G4Input::GetAbsorberYLength(){
	return fAbsorber_y;
}

Double_t G4Input::GetAbsorberZLength(){
	return fAbsorber_z;
}

TVector3 G4Input::GetScattererPosition(){
	return *fPosition_scatterer;
}

TVector3 G4Input::GetAbsorberPosition(){
	return *fPosition_absorber;
}

//Getter for the real event

Double_t G4Input::GetRealPrimaryEnergy(){
	return fPrimaryEn; 
}

Double_t G4Input::GetRealPhotonEnergy(){
	return fEnergy_p; 
}

Double_t G4Input::GetRealElectronEnergy(){
	return fEnergy_e;
}

TVector3 G4Input::GetRealPhotonPosition(){
	return *fPosition_p;
}

TVector3 G4Input::GetRealSourcePosition(){
	return *fPosition_s;
}

TVector3 G4Input::GetRealPhotonDirection(){
	return *fDirection_p;
}

TVector3 G4Input::GetRealPrimaryDirection(){
	return *fDirection_s;
}

TVector3 G4Input::GetRealElectronPosition(){
	return *fPosition_e;
}
...
Double_t G4Input::GetRecoPhotonEnergy(){
	return fReco_en_p->value;
}

Double_t G4Input::GetRecoElectonEnergy(){
	return fReco_en_e->value;
}

TVector3 G4Input::GetRecoPhotonPosition(){
	return fReco_pos_p->position;
}

TVector3 G4Input::GetRecoElectronPosition(){
	return fReco_pos_e->position;
}

TVector3 G4Input::GetRecoPhotonDirection(){
	return fReco_dir_p->position;
}

Double_t G4Input::GetRecoPhotonEnergyUn(){
	return fReco_en_p->uncertainty;
}

Double_t G4Input::GetRecoElectonEnergyUn(){
	return fReco_en_e->uncertainty;
}

TVector3 G4Input::GetRecoPhotonPositionUn(){
	return fReco_pos_p->uncertainty;
}

TVector3 G4Input::GetRecoElectronPositionUn(){
	return fReco_pos_e->uncertainty;
}

TVector3 G4Input::GetRecoPhotonDirectionUn(){
	return fReco_dir_p->uncertainty;
}
