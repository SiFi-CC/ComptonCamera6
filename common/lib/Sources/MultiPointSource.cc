...
// -----------------------
  ...
  MultiPointSource::MultiPointSource(const TString fname){
    ...
    if(!fInFileName.EndsWith(".mac"))
       throw "MultiPointSource constructor: not appropriate input file format, .mac file expected...";
    ...
    if(NormalizeIntensities()==kFALSE)
        throw "MultiPointSource::NormalizeIntensities(): Sum of intensities of all sources = 0, check your input file...";
    CreateHistograms();
}
...
void MultiPointSource::Print(){
    std::cout<<"This MultiPointSource contains "<<fNumOfSources<<" point-like sources:"<<std::endl;
    for(int i=0; i<fNumOfSources; i++){
        std::cout<<" \n\n### Source "<<i+1;
        fSources[i].Print();
    }
