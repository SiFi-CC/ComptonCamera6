...
public:
  ...
  PointSource(const TVector3& position, Double_t energy)
      : Source(position), fEnergy(energy){ CreateHistograms();};
  
  /** Create point source from configuration file, examples in data/sources/
   */
  PointSource(const TString fname);
  
  /** Initializes source properties based on configuration file open elsewhere (needed for use in MultipointSource) */
