...
Track PlanarSource::GenerateEvent() {
  ...
  Track track;
  if(fEnergyDist)  
    track = Track(fPosition+offset, versor, fEnergyDist->GetRandom(fMinEnergy, fMaxEnergy));
  else
...
Bool_t PlanarSource::Init(){
  if(!fInFileName.EndsWith(".mac")){
    std::cout<<"PlanarSource::Init(): not appropriate input file format, .mac file expected..."<<std::endl;
    return kFALSE;
  }
  std::ifstream infile;
  ...
  while(!infile.eof()){
    ...
    if(sitem.Contains("/gps/pos/type")){
      ...
      if(!sitem.Contains("Plane")){
	    std::cout<<"PlanarSource::Init(): Inconsistency in source types. "<<
	               "Plane expected, "<<sitem<<" found in source config file "<<
	...
	std::cout<<"PlanarSource::Init(): Inconsistency in source shapes. "<<
	  "Rectangle expected, "<<sitem<<" found in source config file "<<
	  fInFileName<<std::endl;
      }
    }
	...
	std::cout<<"PlanarSource::Init(): /gps/ene/type "<<sitem<<" not implemented, only Mono and Lin available"<<std::endl;
    if(sitem.Contains("Lin"))
        if(fEnergyDist==NULL) fEnergyDist = new TF1("fEnergyDist","pol1",0,10000);
      }
    }
...
void PlanarSource::Print(){
  std::cout<<"\n\nPlanarSource::Print():\nName =\t"<<GetName()<<"\nEnergy =\t"<<fEnergy<<" MeV"<<"\nPosition=\t("<<fPosition.X()<<" , "<<fPosition.Y()<<" , "<<fPosition.Z()<<") mm"
        <<"\nSize=\t("<<0<<" , "<<fHalfY*2<<" , "<<fHalfZ*2<<") mm\nAngularRange =\t("<<fMinAngle<<" , "<<fMaxAngle<<") rad wrt -X axis"<<std::endl; 
  if(fEnergyDist)
    std::cout<<"Energy generated according to distribution f(E) = "<< 
    fEnergyDist->GetParameter(0)<<" + E*"<<fEnergyDist->GetParameter(1)<<
    " in the range ("<<fMinEnergy<<" , "<<fMaxEnergy<<") MeV\n"<<std::endl;
}
