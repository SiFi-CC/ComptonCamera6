...
PhysicVar PhysicVar::Sum(const std::vector<PhysicVar*> others)
{
    for (int i = 0; i < others.size(); i++)
    {
        ...
        {
            uncertainty =
                uncertainty * uncertainty + others[i]->uncertainty * others[i]->uncertainty;
        }
        else
        {
            uncertainty += others[i]->uncertainty * others[i]->uncertainty;
...
PhysicVec PhysicVec::operator+(const PhysicVec& a)
{
    position += a.position;
    uncertainty.SetX(
        sqrt(uncertainty.x() * uncertainty.x() + a.uncertainty.x() * a.uncertainty.x()));
    uncertainty.SetY(
        sqrt(uncertainty.y() * uncertainty.y() + a.uncertainty.y() * a.uncertainty.y()));
    uncertainty.SetZ(
        sqrt(uncertainty.z() * uncertainty.z() + a.uncertainty.z() * a.uncertainty.z()));
    return *this;
}
PhysicVec PhysicVec::operator-(const PhysicVec& a)
{
