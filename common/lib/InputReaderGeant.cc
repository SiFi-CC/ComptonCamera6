...
InputReaderGeant::InputReaderGeant(TString path)
    : InputReader(path), fLoadReal(false), fCorrectOnly(false), fPrimaryEnergy(0), fEnergyLoss(0),
      fEnergyScattered(0)
{
...
bool InputReaderGeant::AccessTree(TString name, TString namesetup)
{
    ...
    fTreeSetup->SetBranchAddress("NumberOfSimulatedEvents", &fNumberOfSimulatedEvents);
...
bool InputReaderGeant::LoadEvent(int i)
{
    ...
    if (fLoadReal)
    {
        ...
        {
            if (fRealPosition_p->at(m).X() < (fScattererPosition->X() + fScattererDim->X()))
                interactions[0] = true;
            else if (fRealPosition_p->at(m).X() > (fAbsorberPosition->X() - fAbsorberDim->X()))
                interactions[2] = true;
        }
        for (int m = 0; m < fRealInteractions_e->size(); m++)
        {
            if (fRealPosition_e->at(m).X() < (fScattererPosition->X() + fScattererDim->X()))
                interactions[1] = true;
            else if (fRealPosition_e->at(m).X() > (fAbsorberPosition->X() - fAbsorberDim->X()))
                interactions[3] = true;
        }
        if (interactions[0] == interactions[1]) return false;
        if (interactions[2] == interactions[3]) return false;
        fPositionScat->SetXYZ(fRealPosition_e->at(0).X(), fRealPosition_e->at(0).Y(),
                              fRealPosition_e->at(0).Z());
        fPositionAbs->SetXYZ(fRealPosition_p->at(1).X(), fRealPosition_p->at(1).Y(),
                             fRealPosition_p->at(1).Z());
        fDirectionScat->SetXYZ(fRealDirection_scatter->X(), fRealDirection_scatter->Y(),
    ...
    else
    {
        ...
        fPositionScat->SetXYZ(fRecoPosition_e->position.X(), fRecoPosition_e->position.Y(),
                              fRecoPosition_e->position.Z());
        fPositionAbs->SetXYZ(fRecoPosition_p->position.X(), fRecoPosition_p->position.Y(),
