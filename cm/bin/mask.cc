...
int main(int argc, char** argv) {
  ...
  if (argc != 3) {
    cout << "type: './mask fGenVersion fMask' to start:\n\n 1 - pointlike "
            "source in (0,0,0)\n 2 - uniform distribution along z, on beam "
            "axis, right-angled to it\n 3 - isotropic two point-like sources, "
            "20mm gap\n 4 - isotropic two point-like sources, 40mm gap\n\n and "
            "fMask is a mask code, e.g. 1d_47 (for more options look into "
            "../sources/macros/masks/\n\n"
         << endl;
    return 0;
  }
  TString mask(argv[2]);
  TFile* maskfile =
      new TFile("../sources/macros/masks/hMURA" + mask + ".root", "READ");
  TH2F* h = 0;
  ...
  CMSimulation* sim =
      new CMSimulation("CMSimulation_" + gen + "_MURA" + mask, 1);
