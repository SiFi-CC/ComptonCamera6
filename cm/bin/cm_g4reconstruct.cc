...
{
    CmdLineOption cmdopt_output("Output", "-o", "Output file (string), default: reconstruct.root",
                                "reconstruct.root");
    ...
    if (pargs.size() < 2)
    {
        ...
        spdlog::info("type: './cm_g4reconstruct [DETECTOR] [SIMULATIONS] [-n ITERATIONS] "
                     "[-o OUTPUTFILE]' to "
                     "start,\n"
                     " where:\n"
    ...
    spdlog::info("Extracted {} inputs from data file", geometryData.recoData.size());
    geometryData.Print();
