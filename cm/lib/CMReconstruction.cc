...
CMReconstruction::CMReconstruction(TString simulationFile)
{
    ...
    log->debug("Load from \"mask\" object of type {}", file.Get("mask")->ClassName());
    fMask = *static_cast<Mask*>(file.Get("mask"));

    log->debug("Load from \"detector\" object of type {}", file.Get("detector")->ClassName());
    fDetPlane = *static_cast<DetPlane*>(file.Get("detector"));

    log->debug("Load from \"H2Detector\" object of type {}", file.Get("H2Detector")->ClassName());
    fImage = *static_cast<TH2F*>(file.Get("H2Detector"));
    fImage.SetDirectory(nullptr);

    log->debug("Load from \"H2Source\" object of type {}", file.Get("H2Source")->ClassName());
    ...
    log->debug("dimensions of reconstructed object [{}]", fObjectCoords.String().c_str());
    log->debug("dimensions of detector image [{}]", fImageCoords.String().c_str());
...
void CMReconstruction::SingleIteration()
{
    ...
    log->debug("SingleIteration Image / (H * f_k) ({}, {})", weightedImage.GetNrows(),
               weightedImage.GetNcols());

    TMatrixT<Double_t> nextIteration = fMatrixHPrime * weightedImage;
    for (int i = 0; i < fObjectCoords.NBins(); i++)
    {
        nextIteration(i, 0) = nextIteration(i, 0) * fRecoObject.back()(i, 0) / fNormS[i];
    }
...
void CMReconstruction::Write(TString filename) const
{
    ...
    SiFi::tools::convertMatrixToHistogram("histH", "histogram of matrix H(probability matrix)",
                                          fMatrixH)
        .Write();
    ...
    can.Divide(static_cast<int>(sqrt(nIterations)) + 1, static_cast<int>(sqrt(nIterations)) + 1);
    canz.Divide(static_cast<int>(sqrt(nIterations)) + 1, static_cast<int>(sqrt(nIterations) + 1));
    cany.Divide(static_cast<int>(sqrt(nIterations)) + 1, static_cast<int>(sqrt(nIterations)) + 1);
    ...
    for (int i = 0; i < nIterations; i++)
    {
        ...
        auto recoIteration = convertMatrixToHistogram(
            ...
            unvectorizeMatrix(fRecoObject[i], fObjectCoords.NBinsX(), fObjectCoords.NBinsY()));
        recoIteration.Write();
        ...
        histProjX[i] =
            recoIteration.ProjectionX(TString::Format("projection Z, iteration %d", i).Data());
        histProjY[i] =
            recoIteration.ProjectionY(TString::Format("projection Y, iteration %d", i).Data());
