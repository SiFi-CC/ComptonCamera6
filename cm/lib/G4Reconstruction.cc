...
{
    log->debug("G4Reconstruction::G4Reconstruction(simulationParams, detectorImage)");
...
{
    return SiFi::tools::vectorizeMatrix(SiFi::tools::convertHistogramToMatrix(detHist));
}
...
TMatrixT<Double_t> G4Reconstruction::ReadFromTTree(TBranch* detBranch)
{
    ...
    for (int i = 0; i < detBranch->GetEntries(); i++)
    {
        ...
        auto detectorBin =
            fParams.detector.getBin(detPosition->X(), detPosition->Y(), detPosition->Z());
...
int G4Reconstruction::SingleIteration()
{
        ...
        fRecoObject.size() > 98)
    {
        ...
        log->info("iter = {}, sigma = {}, relative = {}", fRecoObject.size(), sigma,
                  abs(sigma - fSigma) / fSigma);
        if (abs(sigma - fSigma) / fSigma > 0.01) { fSigma = sigma; }
        else
        {
            return 0;
        }
