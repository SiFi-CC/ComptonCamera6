...
void CMSimulation::RunSimulation(Int_t nEvents)
{
    ...
    while (nEvents > eventsAccepted)
    {
        ...
        eventsProcessed++;
        log->debug("{} events simulated, {} in acceptance", eventsProcessed, eventsAccepted);
        if (eventsProcessed % 100000 == 0) { log->info("{} processed events", eventsProcessed); }
    }
    log->info("Finished simulation: {} events simulated, {} in acceptance", eventsProcessed,
              eventsAccepted);
}
...
void CMSimulation::Print() const
{
    log->info("Mask object: {}\n"
              ...
              "fA = {}\nfB = {}\nfC = {}\nfD = {}\n"
              "fDimZ = {}\nfDimY = {}\n",
              fMask->GetName(), fMask->GetA(), fMask->GetB(), fMask->GetC(), fMask->GetD(),
              fMask->GetDimZ(), fMask->GetDimY(), fDetPlane->GetName(), fDetPlane->GetA(),
              fDetPlane->GetB(), fDetPlane->GetC(), fDetPlane->GetD(), fDetPlane->GetDimZ(),
              fDetPlane->GetDimY());
