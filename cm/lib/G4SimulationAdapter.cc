...
bool G4SimulationAdapter::IsSimulationGeometryEqual(TList* sim1, TList* sim2) {
  ...
  for (auto entryFromFirst : *sim1) {
    ...
    if (fIgnoredKeys.find(std::string(e1->GetName())) != fIgnoredKeys.end()) {
      continue;
    }
    ...
    for (auto entry : *sim2) {
      ...
      if (TString(e1->GetName()) == TString(e2->GetName()) &&
          e1->GetVal() != e2->GetVal()) {
        log->debug("skipping because key={} has different values ({}, {})",
...
void G4SimulationAdapter::VerifyForReconstruct(TFile* simulationFile) {
  auto firstUserInfo =
      static_cast<TTree*>(fSelected->Get("metadata"))->GetUserInfo();
  auto detector =
      static_cast<TTree*>(simulationFile->Get("metadata"))->GetUserInfo();
