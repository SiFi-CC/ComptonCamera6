...
//------------------------------------------------------------------
void CMSimulation::BuildSetup(double maskdist=50, 
			      double maskZsize=300, 
			      double maskYsize=300,
			      double detdist=70, 
			      double detZsize=300, 
			      double detYsize=300){
...
void CMSimulation::SetupSpectra(void){
  ...
  hYZ = new TH2F("hYZ","Y vs Z for vertex",
		 nbinsz,-maskZdim,maskZdim,nbinsy,-maskYdim,maskYdim);
  hYZdetected = new TH2F("hYZdetected","Y vs Z detected",
			 nbinsz,-maskZdim,maskZdim,nbinsy,-maskYdim,maskYdim);
...
Bool_t CMSimulation::GenerateRay(void){
  ...
  case 4 : //isotropic two point-like sources, 40mm gap
    if(gRandom->Uniform(0,1)<0.5)
      fPoint0.SetXYZ(0,0,0);
    else fPoint0.SetXYZ(0,0,-40);
    theta = acos(gRandom->Uniform(-1,1));
    phi=gRandom->Uniform(-TMath::Pi(), TMath::Pi());
    break;
  default:
    cout<<"You have to choose properly the version of event generator"<<endl;
    break;
  }
...
void CMSimulation::Loop(Int_t nev){
  ...
  while(fNev<nev){
    ProcessEvent();
    i++;
    if(fVerbLevel) cout<<"i="<<i<<" events simulated, "<<fNev<<
		     " in acceptance"<<endl;
  }
}
//------------------------------------------------------------------
...
void CMSimulation::SaveGeometryTxt(void){
  ofstream output("results/"+fName+"_geometry.txt", std::ios::out | std::ios::trunc); 
  ...
  if(fVerbLevel) cout << "Geometry of the setup saved in the text file" << endl;
}
//------------------------------------------------------------------
void CMSimulation::BuildTGeometry(void){
  ...
  TGeoVolume *detector = geom->MakeBox("detector",Vacuum,0.001,detDimY,detDimZ);
