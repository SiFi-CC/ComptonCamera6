...
public:
    H2Coords() = default;
    H2Coords(TH2* hist)
        : fNBinsXY(std::make_pair(hist->GetXaxis()->GetNbins(), hist->GetYaxis()->GetNbins())),
    ...
    {
        std::stringstream ss;
        ss << "number of bins: " << fNBins << ", number of bins along x axis " << fNBinsXY.first
           << ", number of bins along y axis " << fNBinsXY.second;
        return ss.str();
    }
