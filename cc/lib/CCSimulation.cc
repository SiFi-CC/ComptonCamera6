...
CCSimulation::CCSimulation(const TString& name, const TString& outputPath, Int_t genVer, Double_t negAngle, Double_t posAngle, Bool_t verbose)
    : fVerbose(verbose), fOutputPath(outputPath), fGenVersion(genVer), fNegAngle(negAngle), fPosAngle(posAngle)
{
...
void CCSimulation::BuildSetup(Double_t scatDist = 50, // mm
                              Double_t scatZ = 80, Double_t scatY = 80, Double_t absDist = 60,
                              Double_t absZ = 300, Double_t absY = 300)
{
...
void CCSimulation::Loop(Int_t nev)
{
    ...
    while (fNev < nev)
    {
        ...
        i++;
        if (i % 1000 == 0)
            cout << "i = " << i << " events simulated, " << fNev << " events in acceptance" << endl;
    }
}
//------------------------------------------------------------------
...
void CCSimulation::BuildTGeometry()
{
    ...
    { // uniform distribution along z axis (beam)
    ...
    { // two isotropic point-like sources along z
    ...
    { // two isotropic point-like sources along y
