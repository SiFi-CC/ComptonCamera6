...
CCSimulation::CCSimulation(TString name, Bool_t verbose){
  ...
  //fN = ncount;
  fVerbose = verbose;
  fFile = new TFile("../work/results/"+name+".root","RECREATE");
...
void CCSimulation::SaveGeometryTxt(void){
  ofstream output(Form("../work/results/CCSimulation_geometry_gen%i_corr_%.0f_%.0f_%.0f_no.%i.txt",fGenVersion,fXofSource,fYofSource,fZofSource,fNev), std::ios::out | std::ios::trunc);
...
void CCSimulation::BuildTGeometry(void){
   ...
   geom->CloseGeometry();
   geom->SetVisLevel(4);
   geom->Export(Form("../work/results/CCSimulation_TGeometry_gen%i_corr_%.0f_%.0f_%.0f_no.%i.root",fGenVersion,fXofSource,fYofSource,fZofSource,fNev));
}
//------------------------------------------------------------------ 
