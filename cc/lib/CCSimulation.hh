...
///3. Two isotropic point-like sources located along Z axis and separated by distance fZgap.
///fZgap by default is set to 20 mm. It can be changed via SetGapZ(Double_t gapZ) function.
///4. Two isotropic point-like sources located along Y axis and separated by distance fYgap.
///fYgap by default is set to 20mm. It can be changed via SetGapY(Double_t gapY) function.
///5. Circular source of given radius fRadius. By defauls fRadius is 15 mm and it can be changed
///via SetRadius(Double_t radius) function. Center of the source is at (0,0,0).
///
///RESULTS:
///
///Simulated data are saved in the TTree and subsequently in the ROOT file. The tree has the 
///following branches:
...
///
///Details of the simulated setup are saved in text file and as TGeometry in ROOT file.
...
private:
  ...
  Double_t    fYofSource;
  Double_t    fZofSource;
  Double_t    fYgap; 		///<For generator #4 - distance between two point-like sources along Y axis
  Double_t    fZgap;		///<For generator #3 - distance between two point-like sources along Z axis
