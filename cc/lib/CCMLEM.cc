...
Bool_t CCMLEM::Reconstruct(void){
  ...
  //image histogram
  fImage[0] = new TH3F("image","image",fNbinsZ,-fDimZ/2.,fDimZ/2.,fNbinsY,-fDimY/2.,fDimY/2.,fNbinsX,-fDimX/2.,fDimX/2.);
 ...
 */
  ...
  const Double_t maxdist = sqrt(pow(fPixelSizeY,2)+pow(fPixelSizeZ,2)+pow(fPixelSizeX,2));
  ...
  for(Int_t i=fStart; i<fStop; i++){
    ...
    ComptonCone *cone = new ComptonCone(point1,point2,energy1+energy2,energy2);
...
void CCMLEM::Print(void){
 cout << "\nCCMLEM::Print()" << endl;
 cout << setw(35) << "Name of input file: \t" << "../work/results/" + fInputName << endl;
 //cout << setw(35) << "Center of reco plane: \t" << fXofRecoPlane << ", " 
     // << fYofRecoPlane << ", " << fZofRecoPlane << endl;
 cout << setw(35) << "Size of image volume: \t" << fDimZ << ", " << fDimY << ", " << fDimX << endl;
