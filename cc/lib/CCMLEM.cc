...
Bool_t CCMLEM::Reconstruct(void) {
  ...
  // image histogram
  fImage[0] =
      new TH3F("image", "image", fNbinsZ, -fDimZ / 2., fDimZ / 2., fNbinsY,
               -fDimY / 2., fDimY / 2., fNbinsX, -fDimX / 2., fDimX / 2.);
  ...
  */
  ...
  const Double_t maxdist =
      sqrt(pow(fPixelSizeY, 2) + pow(fPixelSizeZ, 2) + pow(fPixelSizeX, 2));
