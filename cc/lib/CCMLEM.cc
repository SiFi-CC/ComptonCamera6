...
CCMLEM::CCMLEM(TString path)
    : fSmear(false), fResolutionX(1.3), fResolutionY(10), fResolutionZ(1.3),
      fLoadReal(false), fCorrectIdentified(false), fIter(1), fFreshOutput(true),
      fStart(0), fStop(1000), fVerbose(false), fNIpoints(0), fPoints(0),
      fDimZ(100), fDimY(100), fDimX(0), fNbinsZ(100), fNbinsY(100), fNbinsX(0),
      fSimpleConvergence(false), fROIX(1), fROIY(1), fROIZ(1),
...
Bool_t CCMLEM::SetInputReader(void) {
  ...
  } else if (file->Get("Setup") && file->Get("Events")) {
    file->Close();
...
/// By calling cones from ComptonCone class, it uses the necessary input data
/// from InputReader, In the loop, it reconstructs requested numbers of
/// ComptonCones and uses the numerical method to get intersection of
/// ComptonCone with the image plane. For each ComptonCone, the following
/// processes will be done: cases: (1) To fill TClonesArray by intersection
/// points and global bin number, AddIsectionPoint() is used (in the following I
/// will describe this function). (2) The global bin numbers are sorted, then
/// for each touched pixel the length of track is calculated to estimate the
/// reconstructed cone.
Bool_t CCMLEM::Reconstruct(void) {
  ...
  TVector3 *point_e, *point_p, *point_dir, *point_abs_sc, *pointreco_e,
      *pointreco_p;
  ...
  /// skipped events. If you want to change this - remove 'badeventcounter'
  /// variable.
...
Int_t CCMLEM::AddIsectionPoint(TString dir, Double_t x, Double_t y,
  ...
  if (dir != "hor" && dir != "ver") {
    if (fVerbose)
      cout << "Unknown direction of intrsecting line: " << dir << endl;
    return 0;
  }

  if (fabs(y) > fDimY / 2. ||
      fabs(z) > fDimZ / 2. /* || fabs(x) > fDimX / 2.*/) {
...
Bool_t CCMLEM::Iterate(Int_t nstop, Int_t iter) {
  ...
  for (entry = 0; entry < nSMentries; entry++) {
    ...
    // cout<< "bin : "<< binno << " binz : " << binz << " biny : " << biny << "
    // sen123 : "<< fSensitivity->GetBinContent(binz,biny) << endl;
  ...
  /*
   fSenHisto[lastiter + 1] = (TH2F*)hthisiter->Clone(Form("%s_SenHisto_iter%i",
   fImage[0]->GetName(), lastiter + 1 )); fSenHisto[lastiter +
   1]->SetTitle(Form("%s_SenHisto_iter%i", fImage[0]->GetTitle(), lastiter + 1
   ));
...
Bool_t CCMLEM::DetermineConvergence(Int_t iter) {
  ...
  if (fSimpleConvergence == false) {
    ////////////////////////////////// Applying Stopping (convergence) criterion
    ///(pixel-wise) ////////////////////////////////////////////
    /// in ROI (starting points are set and than 10 percent of the field is
    /// checked) are chosen, the maximum intensity(pixel content) is found in
    /// each iteration. Then the absolute difference between two sucessive
    /// iterations are calculated if the relative error is less than 1 percent,
    /// then the iteration is stopped. The first iteration is assumed 100
    /// percent.
    ...
    if (lastiter >= 1) {
      for (int i = roibiny; i < roibiny + rangey; i++) {
        for (int j = roibinz; j < roibinz + rangez; j++) {
          ...
          // cout<< "For convergence is the bin " <<  i << "," << j << "," << c1
          // << " checked" << endl;
      ...
      // cout <<
      // "---------------------------------------------------------------------------"
      // <<endl; cout<< diffmax << "," << val << " = " << diffmax/val<< endl;
      // cout << "-------------------bigger than stopping
      // value------------------------------" <<endl;

      if (fSigma[iter] <= fConvergenceCriterium) { return false; }
    }
  }
  // THIS IS IMPLEMENTED AS IT WAS BEFORE NOT CHECKING HOW MEANINGFUL IT IS
    ...
    */
    ...
    if ((fabs(fSigma[iter] - fSigma[iter - 10])) / fabs(fSigma[iter]) <
        fConvergenceCriterium) {
...
void CCMLEM::DrawAtConvergence(int iter) {
  TCanvas* can =
      new TCanvas("MLEM2D_Convergence", "MLEM2D_Convergence", 1000, 1000);
  TCanvas* canz =
      new TCanvas("MLEM1DZ_Convergence", "MLEM1DZ_Convergence", 1000, 1000);
  TCanvas* cany =
      new TCanvas("MLEM1DY_Convergence", "MLEM1DY_Convergence", 1000, 1000);
  // HOLDs
  // Image[0],GausFiltered[0],Image[atconvergence],GausFiltered[atconvervence]
  can->Divide(2, 2);
...
void CCMLEM::DrawREGraph(int iter) {
  ...
  TCanvas* can =
      new TCanvas("RE vs. Iteration", "RE vs. Iteration", 1000, 1000);
  can->cd(1);
...
Bool_t CCMLEM::DrawCanvas(void) {
  ...
  TF1* func = new TF1(
      "func", "[0]/2*TMath::Erf((x-[1])/[4])-[2]/2*TMath::Erf((x-[3])/[4])",
      -10, 10);
  ...
  func->SetParNames("Constant_L", "Mean_value_L", "Constant_R", "Mean_value_R",
                    "Sigma_z");
  ...
  TF1* func_z = new TF1(
      "func_z", "[0]/2*TMath::Erf((x-[1])/[4])-[2]/2*TMath::Erf((x-[3])/[4])",
      -20.5, 20.5);
  ...
  func_z->SetParNames("Constant_L", "Mean_value_L", "Constant_R",
                      "Mean_value_R", "Sigma_z");

  TF1* func_z2 = new TF1(
      "func_z2", "[0]/2*TMath::Erf((x-[1])/[4])-[2]/2*TMath::Erf((x-[3])/[4])",
      -20.5, 20.5);
  ...
  func_z2->SetParNames("Constant_L", "Mean_value_L", "Constant_R",
                       "Mean_value_R", "Sigma_z");

  TF1* func_z3 = new TF1(
      "func_z3", "[0]/2*TMath::Erf((x-[1])/[4])-[2]/2*TMath::Erf((x-[3])/[4])",
      -20.5, 20.5);
  ...
  func_z3->SetParNames("Constant_L", "Mean_value_L", "Constant_R",
                       "Mean_value_R", "Sigma_z");
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////
  TF1* func_y = new TF1(
      "func_y", "[0]/2*TMath::Erf((x-[1])/[4])-[2]/2*TMath::Erf((x-[3])/[4])",
      -5.5, 5.5);
  ...
  func_y->SetParNames("Constant_L", "Mean_value_L", "Constant_R",
                      "Mean_value_R", "Sigma_y");
...
Bool_t CCMLEM::ReadConfig(TString path) {
  ...
  while (!config.eof()) {
    ...
    // USED to set that the Monte Carlo data is loaded from the Geant4
    // simulation instead of the reco
    else if (comment.Contains("Load Real")) {
      config >> fLoadReal;
    }
    // USED to set that only the correct identified events are loaded from the
    // geant4 or NN input
...
Bool_t CCMLEM::GetSigmaError(void) {
  ...
  for (int j = 20; j <= fIter + 1; j = j + 10) {
    ...
    if (fabs(sigma[j]) != 0)
      fSigma[250] = (fabs(sigma[j] - sigma[j - 10])) / fabs(sigma[j]);
    // cout << fSigma[100] << endl;
    if (fSigma[250] < 0.1) {
...
TH2* CCMLEM::SmoothGauss(TH2* hin, double sigma) {
  ...
  if (sigma <= 0) {
    std::cout << "Smearing with sigma = " << sigma
              << " will not work, provide a positive number here..."
              << std::endl;
    return nullptr;
  }

  TH2* hout = dynamic_cast<TH2*>(
      hin->Clone(Form("%s_smooth_%.2f", hin->GetName(), sigma)));
  hout->Reset();
...
void CCMLEM::Print(void) {
  ...
  cout << setw(35) << "Fitting parameters: \t" << fP0 << ", " << fP1 << ", "
       << fP2 << endl;
  cout << setw(35) << "INFORMATION FOR GEANTINPUT" << endl;
  cout << setw(35) << "Loading Real: \t" << fLoadReal << endl << endl;
  cout << setw(35) << "Loading Only correct Indentified Events: \t"
       << fCorrectIdentified << endl
       << endl;
}
//--------------------------------------
