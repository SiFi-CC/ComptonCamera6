...
CCMLEM::CCMLEM(TString path)
    : fSmear(false), fResolutionX(1.3), fResolutionY(10), fResolutionZ(1.3), fLoadReal(false),
      fCorrectIdentified(false), fIter(1), fFreshOutput(true), fStart(0), fStop(1000),
      fVerbose(false), fNIpoints(0), fPoints(0), fDimZ(100), fDimY(100), fDimX(0), fNbinsZ(100),
      fNbinsY(100), fNbinsX(0), fSimpleConvergence(false), fROIX(1), fROIY(1), fROIZ(1),
...
Bool_t CCMLEM::Reconstruct(void)
{
    ...
    for (Int_t i = fStart; i < fStop; i++)
    {
        ...
        for (j = 0; j < fNbinsY + 1; j++)
        {
            ...
            b = 2 * ((-coneAxis.Z()) *
                         ((interactionPoint.Z()) * (-coneAxis.Z()) - y * ((-coneAxis.Y())) +
            ...
            c1 = pow(interactionPoint.Y(), 2) + pow(interactionPoint.X(), 2) +
                 ...
                 ((-4) * (interactionPoint.Y()) * (interactionPoint.X()) * (-coneAxis.Y()) *
                  (-coneAxis.X())) +
            ...
            c2 = (-2) * (interactionPoint.X()) * x +
                 ...
                 2 * pow(-coneAxis.Z(), 2) * pow(x, 2) + 4 * (-coneAxis.X()) * pow(x, 2) -
                 2 * pow(-coneAxis.X(), 2) * pow(x, 2);

            c3 = pow(y, 2) * (1 - 2 * pow(-coneAxis.Z(), 2) - 2 * pow(-coneAxis.Y(), 2)) +
                 y * ((interactionPoint.Y()) *
                          ((-2) + 4 * pow(-coneAxis.Z(), 2) + 4 * pow(-coneAxis.Y(), 2)) +
                      4 * (-coneAxis.Y()) *
                          ((interactionPoint.X()) * (-coneAxis.X()) + x - (-coneAxis.X()) * x));

            c4 = (pow(y, 2) - 2 * y * (interactionPoint.Y()) + pow(interactionPoint.Y(), 2) +
                  pow((interactionPoint.X() - x), 2)) *
                 ((-1) + 2 * pow(K, 2));
        ...
        for (k = 0; k < fNbinsZ + 1; k++)
        {
            ...
            e = 2 * ((-coneAxis.Y()) *
                         ((interactionPoint.Z()) * (-coneAxis.Z()) - z * ((-coneAxis.Z())) +
                          (interactionPoint.Y()) * (-coneAxis.Y()) +
                          (interactionPoint.X()) * (-coneAxis.X()) + x - (-coneAxis.X()) * x) -
            ...
            f1 = pow(interactionPoint.Z(), 2) + pow(interactionPoint.X(), 2) +
                 ...
                 ((-4) * (interactionPoint.Z()) * (interactionPoint.X()) * (-coneAxis.Z()) *
                  (-coneAxis.X())) +
                 ((-2) * pow(interactionPoint.X(), 2) * pow(-coneAxis.X(), 2));
            ...
            f2 = (-2) * (interactionPoint.X()) * x +
                 ...
                 4 * (interactionPoint.X()) * pow(-coneAxis.X(), 2) * x - pow(x, 2) -
                 2 * pow(-coneAxis.Y(), 2) * pow(x, 2) + 4 * (-coneAxis.X()) * pow(x, 2) -
                 2 * pow(-coneAxis.X(), 2) * pow(x, 2);

            f3 = pow(z, 2) * (1 - 2 * pow(-coneAxis.Z(), 2) - 2 * pow(-coneAxis.Y(), 2)) +
                 z * ((interactionPoint.Z()) *
                          ((-2) + 4 * pow(-coneAxis.Z(), 2) + 4 * pow(-coneAxis.Y(), 2)) +
                      4 * (-coneAxis.Z()) *
                          ((interactionPoint.X()) * (-coneAxis.X()) + x - (-coneAxis.X()) * x));

            f4 = (pow(z, 2) - 2 * z * (interactionPoint.Z()) + pow(interactionPoint.Z(), 2) +
                  pow((interactionPoint.X() - x), 2)) *
                 ((-1) + 2 * pow(K, 2));
