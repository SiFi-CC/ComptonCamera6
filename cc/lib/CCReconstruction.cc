...
CCReconstruction::CCReconstruction(TString inputName, TString name,
                                   Bool_t verbose) {
...
//------------------------------------------------------------------
/// Default destructor.
CCReconstruction::~CCReconstruction() { delete fReader; }
//------------------------------------------------------------------
...
ComptonCone* CCReconstruction::ReconstructCone(Int_t i) {
  ...
  if ((TMath::Abs(ang - theta) > epsilon)) {
    ...
    cout << "Event " << i << ": ang = " << ang * TMath::RadToDeg()
         << " deg \t theta = " << theta * TMath::RadToDeg()
         << " deg \t delta = "
         << ang * TMath::RadToDeg() - theta * TMath::RadToDeg() << " deg"
         << endl;
    return NULL;
  }
  ...
  if (fVerbose) {
    ...
    cout << "\taxis:\t";
    axis.Print();
    cout << "\tenergy0 = " << fEnergy0 << "\tenergy1 = " << fEnergy1
         << "\tenergy2 = " << fEnergy2 << "\n";
    cout << "\ttheta = " << theta << " radians\t" << theta * TMath::RadToDeg()
         << " deg\n\t";
  }
...
Bool_t CCReconstruction::ReconstructImage(Int_t iStart, Int_t iStop) {
  ...
  fImage = new TH2F("image", "image", nbinsz, -zlimit, zlimit, nbinsy, -ylimit,
                    ylimit);
...
//------------------------------------------------------------------
/// Resests values of chosen private members of the class to their default
/// values. Varaibles reset in this function: fPoint0, fPoint1, fPoint2,
/// fVersor1, fVersor2, fEnergy0, fEnergy1, fEnergy2.
...
Bool_t CCReconstruction::SaveHistogram(TH1F* h) {
  ...
  file->Close();
  if (fVerbose)
    cout << "\nHistogram " << h->GetName() << " saved in the file " << name
         << endl;
  return kTRUE;
}
//------------------------------------------------------------------
