...
CCReconstruction::CCReconstruction(TString inputName, TString name, Bool_t verbose)
{
    ...
    if (!SetInputReader(inputName)) { throw "##### Exception in CCReconstruction constructor!"; }
...
void CCReconstruction::RebuildSetupTxt(void)
{
    ...
    ifstream input(Form("../work/results/CCSimulation_geometry_gen%i.txt", fGenVersion),
                   std::ios::in);
    if (!(input.is_open()))
    {
...
ComptonCone* CCReconstruction::ReconstructCone(Int_t i)
{
    ...
    if ((TMath::Abs(ang - theta) > epsilon))
    {
        ...
        cout << "Event " << i << ": ang = " << ang * TMath::RadToDeg()
             ...
             << " deg \t delta = " << ang * TMath::RadToDeg() - theta * TMath::RadToDeg() << " deg"
             << endl;
        return NULL;
    }
...
Bool_t CCReconstruction::SaveHistogram(TH1F* h)
{
    ...
    h->Write();
    file->Close();
    if (fVerbose) cout << "\nHistogram " << h->GetName() << " saved in the file " << name << endl;
    return kTRUE;
}
//------------------------------------------------------------------
